...
least_absolute_deviation <- function(model) {
  \(parameters) {-sum(abs(model$y - do.call(model$equation, append(list(model$x), parameters))))}
}

#' @export
ordinary_least_squares <- function(model) {
  \(parameters) {-sum((model$y - do.call(model$equation, append(list(model$x), parameters)))^2)}
}
